...
import { AnimalEnrollment } from './entities/AnimalEnrollment';
...
@ApiUseTags("畜类养殖产方登记")
@ApiBearerAuth()
@Controller('animalEnrollment')
export class AnimalEnrollmentController {
  constructor() {}
  ...
  async getById(@Req() req, @Param('id') id) {
    id = parseInt(id,10);
    const enrollment = await AnimalEnrollment.findOne(id);
    if (!enrollment) throw new NotFoundException();
    const user:User = req.user;
    if (await AnimalEnrollmentController.hasPolicy(user,enrollment))
      return enrollment;
  ...
  @ApiImplicitBody({name:"AnimalEnrollment",type:AnimalEnrollment})
  @UsePipes(
    new ValidationPipe({
  ...
  async create(@Req() req, @Body() input) {
    let enrollment = new AnimalEnrollment(input);
  ...
  async put(@Req() req, @Param('id') id, @Body() input) {
    id = parseInt(id,10);
    let enrollment = await AnimalEnrollment.findOne(id);
    if (!enrollment) throw new NotFoundException();
    if (await AnimalEnrollmentController.hasPolicy(req.user,enrollment)){
  ...
  async getAllOrders(@Req() req,@Param('id') enrollmentId):Promise<EnrollmentOrder[]>{
    ...
    const enrollment = await AnimalEnrollment.findOne(enrollmentId);
    if(!enrollment) throw new NotFoundException("enrollment not found.");
    if(!AnimalEnrollmentController.hasPolicy(req.user,enrollment)) throw new ForbiddenException();
    return await EnrollmentOrder.find({type:EnrollmentType.Animal,enrollmentId});
  }
  ...
  async replaceOrder(@Req() req,@Param('enrollmentId') enrollmentId,@Param('orderId') orderId,@Body() orderInfo:EnrollmentOrder):Promise<EnrollmentOrder>{
    ...
    if(!enrollment) throw new NotFoundException("enrollment not found.");
    if(!AnimalEnrollmentController.hasPolicy(req.user,enrollment)) throw new ForbiddenException();
    const order = await EnrollmentOrder.findOne({id:orderId,type:EnrollmentType.Animal,enrollmentId:enrollmentId})
  ...
  static async hasPolicy(user:User,enrollment:AnimalEnrollment){
